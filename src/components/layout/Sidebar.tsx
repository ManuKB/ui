  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar__nav">
        {NAV.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `navitem ${isActive ? 'is-active' : ''}`}>
            {({ isActive }) => (
              <>
                {isActive && (
                ...
                {item.badgeKey === 'pending' && pendingCount > 0 && <span className="navitem__badge">{pendingCount}</span>}
              </>
            )}
          </NavLink>
        ))}
